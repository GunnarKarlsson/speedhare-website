...
export function getRaces(page: number, pageSize: number) {
  ...
  return fetchJson<Paginated<RaceListItem>>(`${API}/races?${q}`).then((data) => ({
    ...data,
...
export function getRaceBySlug(slug: string) {
  return fetchJson<RaceDetail>(`${API}/races/by-slug/${encodeURIComponent(slug)}`).then(
    normalizeRaceDetail,
  );
}
...
export function getRaceResults(
  ...
  return fetchJson<Paginated<RunnerResultRow>>(`${API}/races/${raceId}/results?${q}`);
}
...
export function searchRacesByRunner(q: string) {
  ...
  return fetchJson<SearchResponse>(`${API}/search/races_by_runner?${params}`).then((data) => ({
